<template>
  ...
  <v-data-table
    :headers="headers"
    :items="proyectos"
    :sort-by="[{ key: 'proyectos', order: 'asc' }]"
  >
    <template v-slot:top>
      <v-toolbar
        flat
      >
        <v-toolbar-title>Tabla Proyectos</v-toolbar-title>
        <v-divider
          class="mx-4"
          inset
          vertical
...
data: () => ({
  ...
  headers: [
    ...
    { title: 'ID Proyecto', key: 'idProyecto' },
    { title: 'Nombre', key: 'nombre' },
    { title: 'Descripcion', key: 'descripcion' },
    { title: 'Donatario (Persona ID)', key: 'DonatarioId' },
...
methods: {
  ...
  editItem (item) { //Edita el registro
    ...
    async deleteItemConfirm () {
      ...
      try {
        await this.axiosRequest(`https://localhost:3001/proyecto/${itemId}`, 'delete');
    ...
    async save () { //Crea y edita registros.
      try {
        if (this.editedIndex > -1) {
          ...
          const response = await this.axiosRequest(`https://localhost:3001/proyecto/${this.editedItem.id}`, 'put', this.editedItem);
          console.log('Elemento editado con exito');
          this.proyectos[this.editedIndex] = response;
        } else {
          // Insertar nuevo registro
          const response = await this.axiosRequest('https://localhost:3001/proyecto/', 'post', this.editedItem);
