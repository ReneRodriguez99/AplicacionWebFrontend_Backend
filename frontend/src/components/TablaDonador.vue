<template>
  ...
  <v-text-field v-model="search" label="Buscar donacion por ID" @input="filtrarID"></v-text-field>


  <v-data-table
    :headers="headers"
    :items="donadores"
    :sort-by="[{ key: 'donadores', order: 'asc' }]"
  >
    <template v-slot:top>
      <v-toolbar
        flat
      >
        <v-toolbar-title>Tabla Donadores</v-toolbar-title>
        <v-divider
          class="mx-4"
          inset
          vertical
...
data: () => ({
  ...
  headers: [
    ...
    { title: 'ID Persona (Donador)', key: 'PersonaId' },
...
methods: {
  ...
  editItem (item) { //Edita el registro
    ...
    async deleteItemConfirm () {
      ...
      try {
        await this.axiosRequest(`https://localhost:3001/donador/${itemId}`, 'delete');
